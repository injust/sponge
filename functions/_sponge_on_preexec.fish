function _sponge_on_preexec --on-event=fish_preexec --argument-names command
    _sponge_clear_state

    set -g _sponge_current_command $command
    ...
    if test (count $found_entries) -ne 0; and not contains $command $_sponge_queue
        set -g _sponge_current_command_previously_in_history true
    else
